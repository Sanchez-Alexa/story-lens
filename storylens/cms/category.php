...
$sql = "SELECT cat_id, name, description FROM category WHERE cat_id=:id;";
$category = pdo($pdo, $sql, [$id])->fetch(); // Get category data
if (!$category) {
    include 'page-not-found.php';       // Page not found
}

$sql = "SELECT a.art_id, a.title, a.summary, a.category_id, a.member_id,
        c.name AS category,
...
$sql = "SELECT cat_id, name FROM category WHERE navigation = 1;";
$navigation = pdo($pdo, $sql)->fetchAll();       // Navigation categories

$section = $category['cat_id'];          // Current category
$title = $category['name'];          // HTML <title>
...
<article class="summary">
    <a href="article.php?id=<?= $article['art_id'] ?>">
